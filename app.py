...
if "pdf_data" not in st.session_state:
    st.session_state.pdf_data = {}  # {filename: bytes}
...
with st.sidebar:
    ...
    if not api_key:
        api_key = st.text_input(
            "🔑 Gemini API Key",
            type="password",
            help="aistudio.google.com → Get API Key (무료)",
        )
    ...
    st.caption("[API Key 발급](https://aistudio.google.com) | 무료")
...
if prompt:
    if not api_key:
        st.warning("⚠️ 사이드바에 Gemini API Key를 입력해 주세요. (aistudio.google.com 에서 무료 발급)")
        st.stop()
...

{
  ...
  "citations": [
    {"file": "파일명.pdf", "excerpt": "해당 파일에서 인용된 핵심 내용 1~2문장"}
  ]
}
...
- 답변은 한국어로 작성하세요."""
                ...
                # PDF를 inline_data로 전달
                parts = [system_prompt]
                for fname, pdf_bytes in st.session_state.pdf_data.items():
                    parts.append(f"\n\n[문서: {fname}]")
                    parts.append({
                        "inline_data": {
                ...
                if result.get("found"):
                    ...
                    for c in citations:
                        ...
                        citation_parts.append(f"<b>{fname}</b> — {excerpt}")
